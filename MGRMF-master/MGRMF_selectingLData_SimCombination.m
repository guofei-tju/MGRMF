...
ks_range = [50,100,200,300,400,500,600];
ks = 300;Iteration_max = 10; gamma_gip = 0.5; lamda_L1s = 0.1;lamda_L2s = 2^5; lamda_1s =2^5; lamda_2s = [2^5]; p_nearest_neighbors = 1; gammas = 5;
...
%for Iteration_max=1:15
            ...
            for lamda_L2 = lamda_L2s
                ...
                for lamda_2 = lamda_2s
                    for nK1 = 4%1:size(K1_list,3)+1
                        for nK1_listNum = 1%1:nchoosek(size(K1_list,3)+1,nK1)
                            nK1_list = nchoosek(1:size(K1_list,3)+1,nK1);
                            for nK2 = 1:size(K2_list,3)+4
                                for nK2_listNum = 1:nchoosek(size(K2_list,3)+4,nK2)
                                    nK2_list = nchoosek(1:size(K2_list,3)+4,nK2);
                                    for run=1:nruns 
                                        rand('seed', seed);
    ...
    %                                         K2(:,:,4)=kernel_corr(y_train,2,0,1);
                                    ...
                                    results = cat(1,results,...
                                            ...
                                            save_results([dataname,'_results_drug5Features.txt'],results);              
                                end   
                            end
                        end
                    end
